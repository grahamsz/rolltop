...
func (s *Store) SaveRawMessage(userID, accountID int64, mailbox string, uid uint32, raw []byte) (Saved, error) {
	...
	parts := []string{
		"blobs", "users", strconv.FormatInt(userID, 10),
...
func (s *Store) OpenUserBlob(userID int64, rel string) (*os.File, error) {
	...
	prefix := filepath.Join("blobs", "users", strconv.FormatInt(userID, 10)) + string(filepath.Separator)
	if filepath.IsAbs(clean) || !strings.HasPrefix(clean, prefix) || strings.Contains(clean, "..") {
