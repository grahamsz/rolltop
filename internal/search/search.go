...
func Open(path string) (*Service, error) {
	...
	if err != nil {
		...
		doc.AddFieldMappingsAt("language", userField)
...
func (s *Service) IndexMessage(ctx context.Context, msg store.MessageRecord, attachments []AttachmentDoc) error {
	...
	doc := map[string]any{
		...
		"language":         language.NormalizeCode(msg.LanguageCode),
	...
	return s.index.Index(strconv.FormatInt(msg.ID, 10), doc)
}
...
func (s *Service) DeleteMessage(ctx context.Context, messageID int64) error {
	...
	return s.index.Delete(strconv.FormatInt(messageID, 10))
}
...
func buildQuery(userID int64, queryText string, sortMode SortMode, opts SearchOptions) blevequery.Query {
	...
	if parsed.Language != "" {
		q := bleve.NewTermQuery(parsed.Language)
		q.SetField("language")
		parts = append(parts, q)
	}
	if parsed.Filename != "" {
...
var operatorRE = regexp.MustCompile(`(?i)(^|\s)(-?)(has:attachment|is:read|is:unread|is:starred|is:notstarred|lang:("[^"]+"|\S+)|filename:("[^"]+"|\S+)|from:("[^"]+"|\S+)|to:("[^"]+"|\S+)|cc:("[^"]+"|\S+)|subject:("[^"]+"|\S+)|after:("[^"]+"|\S+)|before:("[^"]+"|\S+))`)
...
func parseQuery(queryText string) parsedQuery {
	...
	for _, m := range matches {
		...
		case strings.HasPrefix(lower, "lang:"):
			out.Language = language.NormalizeCode(strings.Trim(operatorValue(token), `"`))
