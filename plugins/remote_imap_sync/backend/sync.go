...
func (w *routineWorker) idleLoop() {
	...
	for {
		...
		err := w.fetcher.WatchMailbox(w.ctx, source, w.item.SourceMailbox, func() {
			w.Trigger("idle")
		})
...
func (w *routineWorker) runOnce(trigger string, failureAttempt int) error {
	...
	search, err := w.fetcher.SearchMailboxUIDsSince(w.ctx, sourceAccount(item), item.SourceMailbox, item.LastSourceUID, item.AfterDate)
	if err != nil {
		return fail(err)
	}
	if item.SourceUIDValidity != 0 && item.SourceUIDValidity != search.UIDValidity {
		...
		search, err = w.fetcher.SearchMailboxUIDsSince(w.ctx, sourceAccount(item), item.SourceMailbox, 0, item.AfterDate)
		if err != nil {
			return fail(err)
		}
	}
